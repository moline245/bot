...
async def add_user_with_referal(chat_id, refer, nickname):
    ...
    cursor.close()
    await bot.send_message(chat_id, 'Welcome 👋', reply_markup=kb)
...
async def sending(message):
    ...
    await bot.send_message(message.chat.id, 'Mailing ended✅')
...
async def users_actions(chat_id, message_id):
    ...
    if len(ban_data) == 0:
        ...
        await bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                    text=f'{hide_link(url)}<b>Banned users</b> 🚫 \n\n' + ban_data,
...
async def ban_user_action(chat_id, bot, user):
    ...
    if ban == 0:
        ...
        if user.isnumeric == False:
            ...
            await bot.send_message(chat_id, f'Active Promo Codes @{user}banned ✅', reply_markup=back_admin_panel_kb)
            ...
            await bot.send_message(chat_id, f'User *{user}* успешноbanned ✅',
                                   reply_markup=back_admin_panel_kb, parse_mode = 'Markdown')

    else:

        await bot.send_message(chat_id, 'User ужеbanned ⚠', reply_markup=back_users_kb)
...
async def unban_user_action(chat_id, bot, user):
    ...
    if ban == 1:
        ...
        if user.isnumeric == False:
            ...
            await bot.send_message(chat_id, f'User @{user} разбанен ✅', reply_markup=back_admin_panel_kb)
            await bot.send_message(chat, f'Вас unbanned, send a command /start ✅')
            ...
            await bot.send_message(chat_id, f'User *{user}* successfully unbanned ✅',
                                   reply_markup=back_admin_panel_kb, parse_mode = 'Markdown')
            await bot.send_message(user, f'Вас unbanned, send a command /start ✅', reply_markup=back_admin_panel_kb)

    else:

        await bot.send_message(chat_id, 'User неbanned ⚠', reply_markup=back_users_kb)
...
async def user_increase_balance(chat_id, user, amount):
    ...
    if user.isnumeric():
        ...
        await bot.send_message(chat_id, f'Balance<b>{user}</b> <b>{amount}</b> 🪙 ✅',
        ...
        await bot.send_message(chat_id, f'Balance@{user} successfully replenished on<b>{amount}</b> 🪙 ✅',
                               reply_markup=back_admin_panel_kb, parse_mode='HTML')
...
async def user_decrease_balance(chat_id, user, amount):
    ...
    if user.isnumeric():
        ...
        await bot.send_message(chat_id, f'C balance <b>{user}</b>  <b>{amount}</b>🪙 ✅',
                               reply_markup=back_admin_panel_kb, parse_mode="HTML")
        ...
        await bot.send_message(chat_id, f'C balance @{user} successfully debited <b>{amount}</b>🪙 ✅',
                               reply_markup=back_admin_panel_kb, parse_mode='HTML')
...
async def add_admin(chat_id, user_nickname):
    ...
    if user_nickname.isnumeric() == True:
        ...
        try:
            ...
            await bot.send_message(chat_id, f'User *{user_nickname}*  ❗',
                                   reply_markup=back_admin_panel_kb, parse_mode = "Markdown")
            ...
            if user_nickname == admin[0]:
                ...
                await bot.send_message(chat_id, f'User *{user_nickname}* appointed admin ✅',
                                       reply_markup=back_admin_panel_kb, parse_mode = 'Markdown')
        ...
        try:
            ...
            await bot.send_message(chat_id, f'User @{user_nickname} already appointed admin ❗',
                                   reply_markup=back_admin_panel_kb)
...
async def delete_admin(chat_id, user_nickname):
    ...
    if user_nickname.isnumeric() == True:
        ...
        await bot.send_message(chat_id, f'User *{user_nickname}* no longer admin ✅',
                               reply_markup=back_admin_panel_kb, parse_mode = 'Markdown')
...
async def withdrawal_reject(chat_id):
    ...
    await bot.send_message(chat_id, 'Your withdrawal request has been rejected❎')
...
async def withdrawal_accept(chat_id):
    ...
    if method == 'qiwi':
        ...
        try:
            ...
            await bot.send_message(chat_id, 'Your withdrawal request has been approved✅')
            ...
            await bot.send_message(chat_id, 'Не managed to withdraw funds❎')
            await bot.send_message(admin[0], 'Не managed to withdraw funds❎\n\n' + f'{ex}')
...
async def forms_action(chat_id, message, c):
    ...
    try:
        ...
        for i in cursor.execute("SELECT user_id, amount, requisites, method FROM forms").fetchall():
            ...
            check_form = InlineKeyboardButton('Проверить balance🔎', callback_data=f'{message.chat.id}:check')
        ...
        await c.answer('No claims found🔎')
...
async def getting_demobalance(chat_id, c):
    ...
    if given == 0:
        ...
        await c.answer('На Demobalance 500 🪙')

    elif given == 1:

        cursor.close()
        await c.answer('You are  already received a demо balance❗')
